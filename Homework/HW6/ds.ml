...
let extend_env_list : string list -> exp_val list -> env ea_result =
  fun ids vs ->
    List.fold_left2 (fun ac id v -> ac >>+ extend_env id v) lookup_env ids vs
...
let fields_of_recordVal: exp_val -> ((string * (bool * exp_val)) list) ea_result =
  function
...
let rec string_of_expval =
  ...
  | ProcVal (id, body, env) -> "ProcVal(" ^ id ^ "," ^ string_of_expr body ^ "," ^ string_of_env' [] env ^ ")"
  | PairVal (v1, v2) -> "PairVal(" ^ string_of_expval v1 ^ "," ^ string_of_expval v2 ^ ")"
  | TupleVal evs -> "Tuple(" ^ String.concat "," (List.map string_of_expval evs) ^ ")"
  | UnitVal -> "UnitVal "
...
string_of_env' ac =
  ...
  | ExtendEnvRec (id, param, body, env) -> string_of_env' ((id ^ ":=Rec(" ^ param ^ "," ^ string_of_expr body ^ ")") :: ac) env
...
let add_sool_extension s =
  let s = String.trim s
  in match String.index_opt s '.' with
  | None -> s ^ ".sool"
  | _ -> s
